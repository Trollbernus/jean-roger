...
if ( (isset($_GET['id_sujet'])) AND ($_GET['id_sujet']>=0) AND (is_numeric($_GET['id_sujet'])) ) { // AND(is_int($_GET['id_sujet']))

	$idsujet=intval($_GET['id_sujet']);

	// verifier que le sujet existe
	if ((existe_sujet($idsujet)==0)) {
		 	header('Location: index.php?section=erreur404');
	}
	// verifier que le sujet n'est pas supprime
	if (supprime_sujet($idsujet)==1) {
		 	header('Location: index.php?section=erreur404');
	}
	...
	foreach($messages as $cle => $message)
	{
		...
		if (!$messages[$cle]['suppr']) {
		    ...
		    $messages[$cle]['ordre'] = format_message($message['ordre']);
			...
			if (strlen(htmlspecialchars($message['lien']))>=2) {
				//echo $message['lien'];
				$messages[$cle]['lien'] = '<a href="' . format_message($message['lien']). '"> <img src="vue/images/icone_lien.gif"></a>';		
			}else{
				$messages[$cle]['lien'] = '';
			}
			...
			if (strlen($messages[$cle]['texte'])>=2) {
				$messages[$cle]['texte'] = format_message($message['texte']) . '<br/><br/>';
			}else{
				$messages[$cle]['texte']='';
			}	
