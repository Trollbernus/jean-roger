...
if (strlen($message['texte'])>=2) {
	$texte = format_message($message['texte']) . '<br/><br/>';
}else{
	$texte='';
}
...
if( (isset($_POST['tpost'])) AND ($_POST['modiftexte']==1) ){
	...
	if (strlen($_POST['tpost'])>=8000) {
		$reponse = "Erreur : texte trop long (limité à 8000 caractères).";
	}else{
		if (strlen($_POST['tpost'])>=1) {
			$textmod=nl2br(htmlspecialchars($_POST['tpost']));
			$affichetextmod='<div class="politburo">'.format_message(nl2br(htmlspecialchars($_POST['tpost']))).'</div>';
		}else{
			$textmod='v';
...
if ( ($okmail==1) AND ($oktexte==1) AND ($oklien==1) AND ($okimage==1) ) { // si tout va bien
